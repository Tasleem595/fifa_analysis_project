...
background:linear-gradient(180deg,#01040a 0%,#07101d 35%,#0d1830 100%)!important;
border-right:1px solid rgba(0,212,255,0.25)!important;
box-shadow:0 0 45px rgba(0,212,255,0.12)!important;
...
background:linear-gradient(135deg,rgba(0,212,255,.14),rgba(124,58,237,.10))!important;
...
background:linear-gradient(135deg,rgba(0,212,255,.22),rgba(124,58,237,.18))!important;
border:1px solid rgba(0,212,255,.30)!important;
box-shadow:0 0 22px rgba(0,212,255,.16)!important;
}
...
.stTabs [data-baseweb="tab-list"]{background:var(--card)!important;border:1px solid var(--border)!important;border-radius:10px!important;padding:4px!important;gap:2px!important;}
.stTabs [data-baseweb="tab"]{color:var(--muted)!important;font-family:'Exo 2',sans-serif!important;font-size:.84rem!important;border-radius:6px!important;}
...
def fig_style(fig, title=None, h=400):
    # Preserve an existing title when one was already set by Plotly Express.
...
with st.sidebar:
    st.markdown("""
    <div style="text-align:center;padding:1.2rem .6rem 1rem;
    background:linear-gradient(135deg,rgba(0,212,255,.12),rgba(124,58,237,.12));
    border-radius:20px;border:1px solid rgba(0,212,255,.15);
    box-shadow:0 0 25px rgba(0,212,255,.08)">
      <div style="font-size:3rem;filter:drop-shadow(0 0 12px rgba(0,212,255,.4))">⚽</div>
      <div style="font-family:'Exo 2',sans-serif;font-size:1.35rem;font-weight:800;color:#00d4ff">
      FIFA 22
      </div>
      <div style="font-family:'Space Mono',monospace;
      font-size:.68rem;color:#94a3b8;letter-spacing:.18em;margin-top:.2rem">
      PLAYER ANALYTICS
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.divider()
    st.markdown("""
    <div style="font-size:.68rem;color:#64748b;text-align:center;font-family:'Space Mono',monospace;line-height:2">
      19,239 Players · 110 Features<br>FIFA 22 · SoFIFA
...
# ═══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        st.markdown("#### Missing Values in Regression Features (from notebook)")
...
Values outside fences → clipped (not deleted)
        ...
        raw = df_out[feat_sel].dropna()   # before clipping
...
# ═══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        fig.add_vline(x=binom_dist.mean(), line_color='#f72585', line_dash='dash',
                      annotation_text=f"Mean={binom_dist.mean():.1f}",
                      annotation_font_color='#f72585', row=1, col=1)
        ...
        k_q = st.slider("Find probability for exactly k elite players:", 0, n_trials, 5)
...
# ═══════════════════════════════════════════════════════════════════
    ...
    if st.button("⚡  PREDICT OVERALL RATING", use_container_width=True):
        ...
        # Predict — use .item() to safely convert to Python float (fixes TypeError)
